...
- [x] 1. Fix `InventoryRepository.validateLedger` forEach bug
...
- [x] 2. Add `MarketInsufficientFundsError`, `MarketInsufficientStockError`, `TradeRecord`, `Wallet` to `src/modules/market/common.ts`
...
- [x] 3. Add `BuildingID.Market` / `'market'` inventory ID constant
...
- [x] 4. Create `Market` building class
...
- [x] 5. Create `MarketService` singleton
...
- [x] 6. Instantiate `Market` in `City` and call `marketService.init`
...
- [x] 7. Refactor `TransportAction.finished()` to delegate to `MarketService.sell`
...
- [x] 8. Create `BuyFromMarketAction` and wire into `BlackSmith`
...
- [x] 9. Build `MarketPanel.vue` and wire into `BuildingsList` / `App.vue`
...
- [x] 10. Add adventurer temporary buy/sell control
...
- [x] 11. Verify end-to-end and `tsc --noEmit`
