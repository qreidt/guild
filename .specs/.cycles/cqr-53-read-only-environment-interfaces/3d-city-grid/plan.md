...
- **Authored cell data (module-level constants):** `BUILDING_PLOTS` (4 hero 2×2 plots),
  ...
  > The scalar `TOWN_HALF_CELLS = 5` became per-axis bounds (`INTERIOR_MIN_I/MAX_I` −8…4,
  > `INTERIOR_MIN_J/MAX_J` −6…6) with derived wall lines, and every array below moved
  > with them: `BUILDING_PLOTS` is now **5** plots (`Apothecary` at `[-8, 1]`, LumberMill
  > relocated `[-11,-1]` → `[-13,-1]`); `ROAD_CELLS` = `(0, -6..6)` ∪ `(-8..4, 0)`;
  > `TRAIL_CELLS` = `(-9..-11, 0)`; `HOUSE_CELLS` = 49 cells; the `FIELD_1`/`FIELD_2`
  > rects and their `GROUND_PATCHES` plates shifted east to z centre −29. The shape of
  > the module — authored data, one-time assembly, `buildOccupancy` as the gate — is
  > unchanged, and `grid.ts` was not touched.
...
> **Note (2026-07-31):** items 1 and 3 describe the pre-expansion map. Since
> [CQR-59](../../cqr-59-apothecary-herbs-and-potions/) Phase A the ring is a rectangle
> (N `i=-9`, E `j=-7`, W `j=+7`, south open), the LumberMill sits at `[-13,-1]`, and a
> fifth hero building — the Apothecary at `[-8, 1]` — sits inside the north gate. Every
> other item still applies as written.
